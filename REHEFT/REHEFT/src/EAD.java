...
public class EAD {
	...
	private static HashMap<Vm, Double> readyTime = new HashMap();
        private static HashMap<Cloudlet, Double> joblevel = new HashMap();
        private static HashMap<Cloudlet, Double> priority = new HashMap();
        private static double i=0;
	private static int vmno, cloudletno;
        private static HashMap<Cloudlet, Double>computationCosts = new HashMap<>();
        private static HashMap<Cloudlet, Double>rank =new HashMap<>();
	public static void main(String[] args) {
        try {
			...
			for(int c=0;c<cloudletno;c++){
				...
				length = (long)(200 + (Math.random()*700));
			...
			for (int t = 0; t < vmno; t++) {
				...
				for(Object vmObject: vmlist){
					...
					if(finishTime < minFinishTime){
						...
						assignedVm = v;
                                                total_energy+= findEnergyConsumption(i,cloudletlength);
					System.out.println("cloudletno" +cloud.getCloudletId());
                                        }
				}
		...
		for (Object cloudletObject : cloudletlist) {
         ...
         static double  findEnergyConsumption(int i , long time){
		...
		double executionEnergy = 0.0;
               if(i%2==0)
                    ...
                    {
                        time=time*2;
                        executionEnergy = 1.4 * Math.pow(1.2, 2)*(time);
                    }
