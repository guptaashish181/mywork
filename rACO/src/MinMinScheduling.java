//@author Nidhi Rehani, National Institute of Technology, Kurukshetra
//Min-Min Scheduling of independent tasks on virtual machines
	...
	public static void main(String[] args) {
		try{
			...
			for(int v=0;v<vmno;v++){
				mips = mip[v];          //(int)Math.ceil((1500 + (Math.random()* 3500)));
			...
			cloudletlist = new ArrayList<Cloudlet>();
                        int mi[]={20,19,15,23,15,23,19,23};
			for(int c=0;c<cloudletno;c++){
				//System.out.println("Enter Cloudlet length");
				length =mi[c]; //(long)(200000 + (Math.random()*700000));
